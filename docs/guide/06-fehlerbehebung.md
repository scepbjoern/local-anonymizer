...
- **„Konfiguration geändert. Bisherige Ergebnisse sind veraltet.“ obwohl du nichts an Kategorien/Glossar/Ignorierliste geändert hast:** Das ist ein reales Erkennungsproblem und keine Fehlbedienung. Prüfe: Hast du **seit der letzten Textanalyse** tatsächlich Kategorien, Glossar- oder Ignorier-Einträge geändert? Falls ja, ist die Warnung korrekt – bitte erneut analysieren. Falls nein und die Warnung trotzdem erscheint, obwohl du nur LLM-Einstellungen (Modell, Endpunkt, Review-Schalter) geändert hast: bitte melden, mit genauer Abfolge der Schritte.
- **Start-Button bleibt deaktiviert, obwohl die 32k-Kontext-Bestätigung als „✓ bestätigt“ angezeigt wird:** Die Bestätigung betrifft ausschliesslich die Kontextgrösse deines Modellservers. Ein separater Hinweis („Ausgangskontrolle im aktuellen Kategorienprofil deaktiviert…“) erscheint zusätzlich, wenn im aktiven Profil keine Kategorie im Modus `all` oder `explicit_eupii` aktiv ist – die Ausgangskontrolle ist dann laut Kategorienprofil bewusst nicht vorgesehen (siehe Kapitel „Kategorien und Erkennungsmodi“). Passe in diesem Fall das Kategorienprofil an, falls die Ausgangskontrolle genutzt werden soll.
